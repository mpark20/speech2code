...
export class CodeRunner implements vscode.Disposable {
    ...
    private stopRunning() {
        if (this._isRunning) {
            ...
            vscode.commands.executeCommand("setContext", "speech2code.codeRunning", false);
    ...
    private async getFinalCommandToRunCodeFile(executor: string, appendFile: boolean = true): Promise<string> {
        ...
        if (this._codeFile) {
            ...
            const pythonPath = cmd.includes("$pythonPath") ? await Utility.getPythonPath(this._document) : "python";
